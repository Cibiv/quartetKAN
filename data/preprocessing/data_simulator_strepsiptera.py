...
model_params_path = sys.argv[2] if len(sys.argv) > 2 else "../raw/zone/test/strepsiptera/model-params-msa/param-table.tsv"
quartet_trees_path = sys.argv[1] if len(sys.argv) > 1 else "../raw/zone/test/strepsiptera/quartet-tree-parameters/"
seq_len = 1000
...
# create data file
data_file_name = sys.argv[4] if len(sys.argv) > 4 else '../processed/zone/test/strepsiptera/sim_freq_' + time.strftime("%Y%m%d_%H%M%S") + '.csv'
